...
def p2pkh(public_key: Wallet, amount: int, data_payload: None | bytes = None) -> TxOut:
    """Turn `public_key` and `amount` into a P2PKH TxOut."""
    if data_payload is None:
        return TxOut(amount=amount, script_pubkey=public_key.get_locking_script())
    script = public_key.get_locking_script()
    script += Script.parse_string("OP_RETURN")
    script.append_pushdata(data_payload)
