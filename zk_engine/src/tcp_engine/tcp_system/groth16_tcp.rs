...
use crate::tcp_engine::tcp_system::TCPSystem;
...
/// Inputs are P2PKH
/// Unlocking scripts are: <Sig> <PubKey> w/ r.len = 33, s.len = 32
/// Total length: 65 + 33 + 9 = 107 = 0x6b
/// Outputs are P2PKH
#[derive(Clone)]
...
impl TCPSystem for UniversalTCPSnark {
    ...
    const KEYS_PATH: &str = "data/tcp_engine/keys/";
    const PROOFS_PATH: &str = "data/tcp_engine/proofs/";

    /// Perform the setup based on the provided `chain_index`
    fn setup(setup_data: SetupData) -> Result<()> {
        let (pk, _vk) = Self::setup(setup_data.input_index, setup_data.output_index).unwrap();
        save_to_file(
            &pk.crh_pp.seed,
    ...
    fn prove(proving_data: ProvingData) -> Result<()> {
        ...
        // Proving data
        let input_index = proving_data.chain_parameters.input_index;
        let output_index = proving_data.chain_parameters.output_index;
        ...
        let proof = Self::prove(input_index, output_index, &pk, &public_input, &witness).unwrap();
